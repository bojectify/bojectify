---
description: Generate a component's SCSS token files (_structural.scss, _variants.scss) from its Figma component. Requires Figma.
...
1. **Resolve the target component.** Determine the component directory from `--component` or from the Figma component name (kebab-cased). Verify `src/components/ui/<level>/<name>/` exists. If not, suggest running `/platform:scaffold-ui-component <name>` first and stop.
...
4. **Generate token files** in `src/components/ui/<level>/<name>/tokens/`:
   ...
   /* Regenerate with: /platform:figma-get-component-tokens <figma-url> --component <name> */
