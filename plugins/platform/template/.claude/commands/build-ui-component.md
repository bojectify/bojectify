...
**Prerequisites:** The component directory must already exist (via `/scaffold-ui-component`) and have populated `tokens/` files (via `/get-component-tokens`).
...
This is detected automatically from the Figma design context — when `/get-component-tokens` skips the root padding (because it matches the `--s-breakpoint-margin-y-*` pattern), use the mixin here. The mixin provides responsive vertical padding that scales with each breakpoint.
