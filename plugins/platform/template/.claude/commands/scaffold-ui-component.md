...
6. **`tokens/_structural.scss`**
   ...
   ```
   /* Component tokens — regenerate with /get-component-tokens */
...
describe('<PascalCaseName> screenshots', () => {
  test('default', async () => {
    ...
    const element = page.getByTestId(QA_<UPPER_SNAKE_CASE_NAME>.COMPONENT);
    await expect(element).toMatchScreenshot('<kebab-case-name>-default');
  });
});
```

11. **`references/`** — empty directory (Figma reference screenshot saved here by `/new-component`).
